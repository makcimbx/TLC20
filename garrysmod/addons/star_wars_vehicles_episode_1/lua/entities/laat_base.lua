...
function ENT:OnRemove(p)
	...
	if(self.Inflight and not self.Done) then
		self:Exit(false,1); -- Let the player out
		self:Exit(false,2); -- Let the player out
		self:Exit(false,3); -- Let the player out
	end
end
...
function ENT:Enter(p)
	...
	--[[if(not self.Inflight) then
		...
		if(self.PilotVisible and self.PilotPosition) then
			...
			self:SpawnPilot(pos,1);
		end
	end]]--
	
		if(not self.TPilot) then
		p:SetNetworkedEntity(self.Vehicle,self); --Set a networked entity as the name of the vehicle
		p:SetNetworkedBool("Flying"..self.Vehicle,true); --Set a bool on the player
		p:Spectate(OBS_MODE_CHASE); --Spectate the vehicle
		p:DrawWorldModel(false);
		...
		p:SetCollisionGroup(COLLISION_GROUP_IN_VEHICLE);
		for k,v in pairs(p:GetWeapons()) do
			table.insert(self.TWeaponsTable, v:GetClass());
		end
		if(IsValid(p:GetActiveWeapon())) then
			self.TPlayerActiveWeapon = p:GetActiveWeapon():GetClass();
		end
		...
		for k,v in pairs(p:GetWeapons()) do
			...
			self.TWeaponAmmo[k] = tbl;
		end
		
		p:StripWeapons();
		self.TPlayerHealth=p:Health();
		if(p:FlashlightIsOn()) then
			p:Flashlight(false); --Turn the player's flashlight off when Flying
		end
		p:SetViewEntity(self)

		self.TPlayerArmor = p:Armor();
		self.TPilot = p;
		self.NextUse.Use = CurTime()+1;
		...
		if(self.TPilotVisible and self.TPilotPosition) then
			local pos = self:GetPos() + self:GetRight() * self.TPilotPosition.x + self:GetForward() * self.TPilotPosition.y + self:GetUp() * self.TPilotPosition.z;
			self:SpawnPilot(pos,2);
		end
	end
end

function ENT:SpawnPilot(pos,v)
	if(v == 1)then
	...
	if(v == 2)then
		if(IsValid(self.TPilot)) then
			local e = ents.Create("prop_physics");
			e:SetModel(self.TPilot:GetModel());
			e:SetPos(pos)
			local ang = self:GetAngles();
			if(self.TPilotAngle) then
				ang = self:GetAngles() + self.TPilotAngle;
			end
			e:SetAngles(ang)
			e:SetParent(self);
			e:Spawn();
			...
			self.TPilotAvatar = e;
			self:SetNWEntity("TPilotAvatar",e);
		end
	end
end
...
function ENT:Exit(kill,v) --####### Get out @RononDex
	...
	if(v == 2)then
		if (IsValid(self.TPilot)) then
			self.TPilot:UnSpectate();
			self.TPilot:DrawViewModel(true);
			self.TPilot:DrawWorldModel(true);
			self.TPilot:Spawn();
			self.TPilot:SetNWEntity(self.Vehicle,NULL)
			self.TPilot:SetNWBool("Flying"..self.Vehicle,false);
			...
			self.TPilot:SetPos(self.ExitPos);
		
			self.TPilot:SetMoveType(MOVETYPE_WALK);
			self.TPilot:SetCollisionGroup(COLLISION_GROUP_PLAYER);
			self.TPilot:SetViewEntity( NULL )
			self.TPilot:SetHealth(self.TPlayerHealth);
			self.TPilot:SetArmor(self.TPlayerArmor);
			for k,v in pairs(self.TWeaponsTable) do
				if(!self.TPilot:HasWeapon(tostring(v))) then
					self.TPilot:Give(tostring(v));
				end
			end
			if(self.TPlayerActiveWeapon != "") then
				self.TPilot:SelectWeapon(self.TPlayerActiveWeapon);
			end
		
			self.TPilot:StripAmmo();
		
			for k,v in pairs(self.TWeaponAmmo) do
				self.TPilot:SetAmmo(v[2],v[1]);
			end
		
			self.TPilot:SetRenderMode(RENDERMODE_NORMAL);
			self.TPilot:SetColor(Color(255,255,255,255));		
			if (kill and self.TPilot:Alive() and !self.TPilot:HasGodMode()) then 
				self.TPilot:Kill();
			end
		end

		self.TPilot:SetVelocity(self:GetVelocity());
	
		table.Empty(self.TWeaponsTable); 
		table.Empty(self.TWeaponAmmo); 
		...
		if(IsValid(self.TPilotAvatar)) then
			self.TPilotAvatar:Remove();
			self:SetNWEntity("TPilotAvatar",NULL);
		end
	end
end
	...
	if(self.Inflight) then
		...
		if(self.Docked and !self.TakeOff) then
			if(self.NextUse.DockCheck < CurTime()) then
				...
				self.NextUse.DockCheck = CurTime() + 2;
			end
		end
...
function ENT:OnTakeDamage(dmg)
	...
	/*
	if(self.VehicleHealth < self.StartHealth*0.3) then
		if(!self.StartedEngineTimer) then
			self.StartedEngineTimer = true;
			timer.Create(self.Vehicle .. "EngineProblems" .. self:EntIndex() * math.random(1,1000), 5, 0, function()
				self.Tractored = true;
...
function ENT:Use(p)
	...
	if(not self.Inflight and (self.NextUse.Use < CurTime())) then
		self:Enter(p);
	end
end
			...
			if(self.Inflight and IsValid(self.Pilot)) then
				...
				if(!self.TakeOff and !self.Land) then
					...
					if(self.Pilot:KeyDown(IN_JUMP) and self.Pilot:KeyDown(IN_DUCK)) then
						local filter_list = {self};
						if(self.TraceFilter) then
							filter_list = self.TraceFilter;
						end
						local tr = util.TraceLine({
					...
					if(self.HasLookaround) then
						if(self.Pilot:KeyPressed(IN_SCORE)) then
					...
					if(self.HasLookaround and self.Pilot:KeyReleased(IN_SCORE)) then
						self.Pilot:SetEyeAngles(self:GetAngles());
					end
					...
					if(self.CanStrafe) then
						fPos = self:GetPos()+(FWD*self.Accel.FWD)+(UP*self.Accel.UP)+(RIGHT*self.Accel.RIGHT);
					end
