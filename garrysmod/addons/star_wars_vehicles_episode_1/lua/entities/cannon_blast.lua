...
if SERVER then
	...
	function ENT:PhysicsCollide(data, physobj)
		...
		for i=1,math.Round(self.Damage/500) do
			local pos = self:GetPos()+self:GetForward()*math.random(-self.Damage/7,self.Damage/7)+self:GetRight()*math.random(-self.Damage/7,self.Damage/7)
			local fx = EffectData()
				fx:SetOrigin(pos);
			util.Effect("Explosion",fx,true,true);
		end
		for k,v in pairs(ents.FindInSphere(self:GetPos(),self.Damage/4)) do
			local dist = (self:GetPos() - v:GetPos()):Length();
			--local dmg = math.Clamp((self.Damage or 600) - dist,0,(self.Damage or 600));
			local dmg = math.Clamp((self.Damage) - dist,0,(self.Damage));
			v:TakeDamage(dmg);
		end
		self:Remove()
	end
