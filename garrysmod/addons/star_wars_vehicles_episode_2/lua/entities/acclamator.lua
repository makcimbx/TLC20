...
ENT.EntModel = "models/acclamator/syphadias/acclamator.mdl"
ENT.Vehicle = "Acclamator"
...
function ENT:SpawnFunction(pl, tr)
	local e = ents.Create("acclamator");
...
if CLIENT then
	...
	function ENT:Think()
		...
		if(Flying) then
			self.EnginePos = {
				self:GetPos()+self:GetForward()*1855+self:GetUp()*-285+self:GetRight()*246,
				self:GetPos()+self:GetForward()*1855+self:GetUp()*-285+self:GetRight()*-246,
			}
			self:Effects();
		end
	...
	local function CalcView()
		...
		local p = LocalPlayer();
		local self = p:GetNWEntity("Acclamator")
		local Flying = p:GetNWBool("FlyingAcclamator");
		local pos,face;
	...
	hook.Add("CalcView", "AcclamatorView", CalcView)
	...
	hook.Add("HUDPaint", "AcclamatorReticle", AcclamatorReticle)
