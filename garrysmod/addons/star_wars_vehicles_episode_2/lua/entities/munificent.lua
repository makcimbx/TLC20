...
ENT.EntModel = "models/Munificent/syphadias/Munificent.mdl"
ENT.Vehicle = "Munificent"
...
function ENT:SpawnFunction(pl, tr)
	local e = ents.Create("munificent");
...
function ENT:Initialize()
	...
	self.WeaponLocations = {
		Bottom = self:GetPos()+self:GetForward()*-2330+self:GetUp()*-20,
		Top = self:GetPos()+self:GetForward()*-2330+self:GetUp()*65
	}
	self.WeaponsTable = {};
	self.BoostSpeed = 150;
	...
	self.FireGroup = {"Bottom","Top"}
	
	self.PilotOffset = {x=0,y=-2330,z=0}
	...
	self.ExitModifier = {x=0,y=-600,z=300};
...
function ENT:FireWeapons()
	...
	if(self.NextUse.Fire < CurTime()) then
		for k,v in pairs(self.Weapons) do
			...
			self.Bullet.Dir = self.Pilot:GetAimVector():Angle():Forward()*-1;
...
if CLIENT then
	...
	function ENT:Think()
		...
		if(Flying) then
			self.EnginePos = {
				self:GetPos()+self:GetForward()*2500+self:GetUp()*320,
				self:GetPos()+self:GetForward()*2500+self:GetUp()*110+self:GetRight()*165,
				self:GetPos()+self:GetForward()*2500+self:GetUp()*110+self:GetRight()*-165,
			}
			self:Effects();
		end
	...
	local function CalcView()
		...
		local p = LocalPlayer();
		local self = p:GetNWEntity("Munificent")
		local Flying = p:GetNWBool("FlyingMunificent");
		local pos,face;
	...
	hook.Add("CalcView", "MunificentView", CalcView)
	...
	function MunificentReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("FlyingMunificent");
		local self = p:GetNWEntity("Munificent");
		if(Flying and IsValid(self)) then
			local WeaponsPos = self:GetPos()+self:GetForward()*-2430+self:GetUp()*-20;
			/*
	...
	hook.Add("HUDPaint", "MunificentReticle", MunificentReticle)
