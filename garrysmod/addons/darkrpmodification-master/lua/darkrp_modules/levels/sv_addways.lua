...
timer.Create( "PlayXP", time,0,function()
	if(LevelSystemConfiguration.TimerModule) then
		for k,v in pairs(player.GetAll()) do 
			local x = 0
			if v:IsUserGroup("admin") then x=4 else if v:IsUserGroup("vip") then x=3 else if v:IsUserGroup("minivip") then x=2 else x=1 end end end
			local XP = v:addXP(LevelSystemConfiguration.TimerXPAmount*x, true)
			if(XP) then
