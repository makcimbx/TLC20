SWEP.Gun					= ("tfa_voxel_scoutblaster") --Make sure this is unique.  Specically, your folder name.
...
SWEP.PrintName				= "Пистолет Газа(ТОЛЬКО ДЛЯ ГАЗА)"		-- Weapon name (Shown on HUD)
...
--Firing related
SWEP.Primary.ReloadSound = Sound ("weapons/synbf3/SCOUTBLASTER_reload.wav");
SWEP.Primary.Sound 			= Sound("weapons/synbf3/SCOUTBLASTER_fire.wav");				-- This is the sound of the weapon, when you shoot.
SWEP.Primary.SilencedSound 			= nil				-- This is the sound of the weapon, when silenced.
SWEP.Primary.PenetrationMultiplier = 1 --Change the amount of something this gun can penetrate through
SWEP.Primary.Damage		= 99999999909					-- Damage, in standard damage points.
...
SWEP.Primary.RPM_Burst				= 1000					-- RPM for burst fire, overrides semi.  This is in Rounds Per Minute / RPM
...
SWEP.CrouchAccuracyMultiplier=0.0005--Less is more.  Accuracy * 0.5 = Twice as accurate, Accuracy * 0.1 = Ten times as accurate
SWEP.ChangeStateAccuracyMultiplier=.0005 --Less is more.  A change of state is when we're in the progress of doing something, like crouching or ironsighting.  Accuracy * 2 = Half as accurate.  Accuracy * 5 = 1/5 as accurate
SWEP.JumpAccuracyMultiplier=.0002--Less is more.  Accuracy * 2 = Half as accurate.  Accuracy * 5 = 1/5 as accurate
SWEP.WalkAccuracyMultiplier=.000035--Less is more.  Accuracy * 2 = Half as accurate.  Accuracy * 5 = 1/5 as accurate
...
SWEP.ViewModel			= "models/weapons/synbf3/c_scoutblaster.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/synbf3/w_scoutblaster.mdl" -- Weapon world model path
...
SWEP.IronSightsPos = Vector(-3.921, -6.231, 2.42)
...
SWEP.ShootWhileDraw=true --Can you shoot while draw anim plays?
...
SWEP.VElements = {
	["element_name"] = { type = "Model", model = "models/rtcircle.mdl", bone = "v_scoutblaster_reference001", rel = "", pos = Vector(01.362, 2.885, 01.7), angle = Angle(180, 90, 0), size = Vector(0.299, 0.299, 0.299), color = Color(255, 255, 255, 255), surpresslightning = false, material = "!tfa_rtmaterial", skin = 0, bodygroup = {} }
}
...
local function  drawFilledCircle( x, y, radius, seg )
	...
	table.insert(kirkle, { x = x + math.sin( a ) * radius, y = y + math.cos( a ) * radius, u = math.sin( a ) / 0.5 + 0.5, v = math.cos( a ) / 2 + 0.5 } )
...
local g36
if surface then
	g36 = surface.GetTextureID("scope/gdcw_elcanreticle") --the texture you vant to use
end
