...
ATTACHMENT.Name = "Настроить снайперский прицел 8.7x"
--ATTACHMENT.ID = "base" -- normally this is just your filename
ATTACHMENT.Description = { TFA.AttachmentColors["="], "8.7x зум ", TFA.AttachmentColors["-"], "На 40% меньше скорости прицеливания ",  TFA.AttachmentColors["-"], "На 10% меньше скорость перемещение при прицеливание" }
...
ATTACHMENT.ShortName = "DC15x"

local fov = 90 / 8.7 / 2 -- Default FOV / Scope Zoom / screenscale
...
ATTACHMENT.WeaponTable = {
	...
	["IronSightsPos"] = function( wep, val ) return wep.IronSightsPos_DC15x or val end,
	["IronSightsAng"] = function( wep, val ) return wep.IronSightsAng_DC15x or val end,
...
ATTACHMENT.FOV = fov
ATTACHMENT.Reticule = "models/weapons/tfa_ins2/optics/mk4_crosshair"
...
function ATTACHMENT:Attach(wep)
	...
	wep.RTCode = function( myself , rt, scrw, scrh)
		...
		local att, ts
		if wep:VMIV() then
			att = wep.OwnerViewModel:GetAttachment( wep.RTAttachment_DC15x or 0 )
		end
		...
		if wep.ScopeAngleTransforms_MX4 then
			ang:RotateAroundAxis(ang:Right(), wep.ScopeAngleTransforms_DC15x.p )
			ang:RotateAroundAxis(ang:Up(), wep.ScopeAngleTransforms_DC15x.y )
			ang:RotateAroundAxis(ang:Forward(), wep.ScopeAngleTransforms_DC15x.r )
		end
		cd.angles = ang
		...
		if ts then
			...
			if wep.ScopeOverlayTransforms_MX4 then
				scrpos.x = scrpos.x + wep.ScopeOverlayTransforms_DC15x[1]
				scrpos.y = scrpos.y + wep.ScopeOverlayTransforms_DC15x[2]
			end
			...
			if wep.ScopeOverlayTransformMultiplier_MX4 then
				scrpos.x = scrpos.x * wep.ScopeOverlayTransformMultiplier_DC15x
				scrpos.y = scrpos.y * wep.ScopeOverlayTransformMultiplier_DC15x
			end
			...
			if wep.RTScopeOffset_MX4 then
				rtow = self.RTScopeOffset_DC15x[1] * rttw
				rtoh = self.RTScopeOffset_DC15x[2] * rtth
			end
			local rtw, rth = rttw * 1, rtth * 1
			if self.RTScopeScale_MX4 then
				rtw = rtw * self.RTScopeScale_DC15x[1]
				rth = rth * self.RTScopeScale_DC15x[2]
			end
			local distfac = math.pow( 1 - math.Clamp( ( att.Pos:Distance( wep.Owner:GetShootPos() ) - ( wep.ScopeDistanceMin_DC15x or 4 ) ) / ( wep.ScopeDistanceRange_DC15x or 10 ), 0, 1 ), 1 )
