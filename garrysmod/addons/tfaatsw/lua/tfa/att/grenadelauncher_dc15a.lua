...
ATTACHMENT.Name = "Подключить подствольный гранатомет с бакта снарядами"
--ATTACHMENT.ID = "base" -- normally this is just your filename
ATTACHMENT.Description = { TFA.AttachmentColors["+"], "E+ЛКМ что-бы переключиться на гранатомет", "На 20% меньше вертикальная отдача", TFA.AttachmentColors["-"], "На 10% снижается кучность стрельбы", "На 5% снижается меткость при прицеливание", "Ниже скорость передвижения" }
...
ATTACHMENT.ShortName = "Bactanade"

ATTACHMENT.Ent = "sent_bactanade"
ATTACHMENT.Damage = 0
			...
			if CurTime() >  myself:GetNextPrimaryFire() and TFA.Enum.ReadyStatus[ myself:GetStatus() ] and not myself:GetSprinting() then
				...
				if myself:Clip2() > 0 then
					if SERVER then
						...
						ent:SetAngles( myself.Owner:EyeAngles() + Angle(90,0,0) )
						ent:Spawn()
						ent:Activate()
