			...
			if (curShield and weapon:GetDTBool(0, false) != true) then
					...
					if (sInfo) then
						if (!IsValid(client.btShieldModel)) then
							...
							local pos, ang = client:GetBonePosition(client:LookupBone(sInfo.bone) or 1)
							if (!pos or !ang) then return end

							local tempAng = Angle(ang)
								ang:RotateAroundAxis(tempAng:Forward(), sInfo.render.ang[1])
								ang:RotateAroundAxis(tempAng:Up(), sInfo.render.ang[2])
								ang:RotateAroundAxis(tempAng:Right(), sInfo.render.ang[3])
								pos = pos
								+ tempAng:Up() * sInfo.render.pos[1]
								+ tempAng:Forward() * sInfo.render.pos[2]
								+ tempAng:Right() * sInfo.render.pos[3]
							tempAng = nil
							...
							if (ONLY_FOR_DEVELOPERS) then
								local pos, ang = client:GetBonePosition(client:LookupBone(sInfo.bone) or 1)
								local tempAng = Angle(ang)
									ang:RotateAroundAxis(tempAng:Forward(), sInfo.block.ang[1])
									ang:RotateAroundAxis(tempAng:Up(), sInfo.block.ang[2])
									ang:RotateAroundAxis(tempAng:Right(), sInfo.block.ang[3])
									pos = pos
									+ tempAng:Up() * sInfo.block.pos[1]
									+ tempAng:Forward() * sInfo.block.pos[2]
									+ tempAng:Right() * sInfo.block.pos[3]
								tempAng = nil
