...
function JukeBox:PlayVideo( id )
	if not (self.HTMLPlayer) then return end
	if not tobool( self:GetCookie( "JukeBox_Enabled" ) ) then return end
	if not JukeBox.Settings.PlayWhileAlive and not JukeBox.PlayerAlive then return end
	self.HTMLPlayer:RunJavascript( 'player.loadVideoById("'..id..'", 0, "'..JukeBox.Quality..'");')
end
...
function JukeBox:PlayVideoFromTime( id, time )
	...
	if not tobool( self:GetCookie( "JukeBox_Enabled" ) ) then return end
	if not JukeBox.Settings.PlayWhileAlive and not JukeBox.PlayerAlive then return end
	self.HTMLPlayer:RunJavascript( 'player.loadVideoById("'.. id .. '", '..time..', "'..JukeBox.Quality..'");')
end
...
function JukeBox:PlayVideoUntilTime( id, time )
	...
	if not tobool( self:GetCookie( "JukeBox_Enabled" ) ) then return end
	if not JukeBox.Settings.PlayWhileAlive and not JukeBox.PlayerAlive then return end
	self.HTMLPlayer:RunJavascript( 'player.loadVideoById({ "videoId" : "'..id..'", "startSeconds" : 0, "endSeconds" : '..time..', "suggestedQuality" : "'..JukeBox.Quality..'" });')
end
...
function JukeBox:PlayVideoWithTimes( id, startTime, endTime )
	...
	if not tobool( self:GetCookie( "JukeBox_Enabled" ) ) then return end
	if not JukeBox.Settings.PlayWhileAlive and not JukeBox.PlayerAlive then return end
	self.HTMLPlayer:RunJavascript( 'player.loadVideoById({ "videoId" : "'..id..'", "startSeconds" : '..startTime..', "endSeconds" : '..endTime..', "suggestedQuality" : "'..JukeBox.Quality..'" });')
end
