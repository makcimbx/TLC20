...
function JukeBox.VGUI.Pages.Queue:CreatePanel( parent )
	...
	parent.Header.Paint = function( self, w, h )
		...
		draw.SimpleText( JukeBox.Lang:GetPhraseCaps( "#HEADING_Song" ), "JukeBox.Font.18", 82, h/2, Color( 200, 200, 200 ), 0, 1 )
		draw.SimpleText( JukeBox.Lang:GetPhraseCaps( "#HEADING_Artist" ), "JukeBox.Font.18", w/2.5, h/2, Color( 200, 200, 200 ), 0, 1 )
		draw.SimpleText( JukeBox.Lang:GetPhraseCaps( "#HEADING_Length" ), "JukeBox.Font.18", w-135, h/2, Color( 200, 200, 200 ), 2, 1 )
	...
	function parent.Scroll:UpdateQueue()
		...
		if table.Count( JukeBox.SongList ) <= 0 then
			...
			for pos, data in pairs( JukeBox.QueueList ) do
				id = data.id
				JukeBox.VGUI.Pages.Queue:CreateSongCard( parent, pos, data )
...
function JukeBox.VGUI.Pages.Queue:CreateSongCard( parent, pos, info ) -- IT GOT A RE-DO!
	if not JukeBox.SongList[info.id] then return end -- The queue'd song doesn't exist in the All Songs list
	local name = JukeBox.SongList[info.id].name
	local artist = JukeBox.SongList[info.id].artist
	local id = JukeBox.SongList[info.id].id
	local length = JukeBox.SongList[info.id].length
	...
	card.Paint = function( self, w, h )
		...
		draw.SimpleText( JukeBox:MinWord( name, "JukeBox.Font.20", w-70-(w-(w/2.5))-20 ), "JukeBox.Font.20", 70, h/2, Color( 255, 255, 255 ), 0, 1 )
		draw.SimpleText( JukeBox:MinWord( artist, "JukeBox.Font.20", w-10-(w/2.5)-144-20 ), "JukeBox.Font.20", w/2.5, h/2, Color( 255, 255, 255 ), 0, 1 )
...
JukeBox.VGUI:RegisterPage( "#TAB_Main", "Queue", "#TAB_Queue", "jukebox/list.png", function( parent ) JukeBox.VGUI.Pages.Queue:CreatePanel( parent ) end )