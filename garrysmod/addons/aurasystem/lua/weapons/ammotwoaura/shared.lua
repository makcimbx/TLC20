...
if timer.RepsLeft( "AMCD" ..self.Owner:UserID() ) != nil or timer.RepsLeft( "AM3CD" ..self.Owner:UserID() ) != nil then
...
for _, hply in pairs(targetheal) do
		if hply:IsPlayer() then 
			if (SERVER) then hply:GiveAmmo( 3, 10 , false) end
		end
	end
	
if self.Cd == nil or 0 then 
timer.Create( "AM2CD" ..self.Owner:UserID(), 1, 45, function () end) else return end
end
end
...
function SWEP:Think()
self.Cd = timer.RepsLeft( "AM2CD" ..self.Owner:UserID() )
self.Activeskil = timer.RepsLeft( "AM2deploy" ..self.Owner:UserID() )
...
function SWEP:DrawHUD()	
	...
	surface.SetMaterial(Material( "skilliconammoraura2.png" ))
