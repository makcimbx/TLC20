...
SWEP.PrintName = "Overarmor aura"
SWEP.Author = "KTKycT (Comet)"
SWEP.Slot = 4
SWEP.SlotPos = 0
SWEP.Description = "AOE пополнение брони союзников."
SWEP.Contact = ""
SWEP.Purpose = ""
...
if timer.RepsLeft( "APCD" ..self.Owner:UserID() ) != nil or timer.RepsLeft( "ULAPOCD" ..self.Owner:UserID() ) != nil then
...
timer.Create( "APover" ..self.Owner:UserID(), 1, 15, function ()
	targetheal = ents.FindInSphere(self.Owner:GetPos(), 140)
	for _, hply in pairs(targetheal) do
		if hply:IsPlayer() and hply:Alive() then 
			if (SERVER) then needarmor = DarkRPG.Server.Player[self.Owner:SteamID()].armor 
				if hply:Armor() >= (needarmor) then
					hply:SetArmor( math.Clamp(hply:Armor() + 7, 0, needarmor + 50))	
			end
		end
	end
end
 ...
 ef = EffectData()
	...
	util.Effect("armoraurarad", ef,true,true)
	end)
	
if self.Cd == nil or 0 then 
timer.Create( "APOCD" ..self.Owner:UserID(), 1, 45, function () end) else return end
end
end
...
function SWEP:Think()
self.Cd = timer.RepsLeft( "APOCD" ..self.Owner:UserID() )
self.Activeskil = timer.RepsLeft( "APover" ..self.Owner:UserID() )
...
function SWEP:DrawHUD()	
	...
	surface.SetMaterial(Material( "skilliconarmoraura2.png" ))
