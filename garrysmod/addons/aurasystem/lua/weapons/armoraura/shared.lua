...
if timer.RepsLeft( "APOCD" ..self.Owner:UserID() ) != nil or timer.RepsLeft( "ULAPOCD" ..self.Owner:UserID() ) != nil then
...
timer.Create( "APregen" ..self.Owner:UserID(), 1, 15, function ()
	...
	for _, hply in pairs(targetheal) do
		if hply:IsPlayer() and hply:Alive() then 
			if (SERVER) then needarmor = DarkRPG.Server.Player[self.Owner:SteamID()].armor 
					hply:SetArmor( math.Clamp(hply:Armor() + 4, 0, needarmor))	
			end
		end
	end
 ef = EffectData()
...
timer.Create( "APCD" ..self.Owner:UserID(), 1, 45, function () end) else return end
end
end
...
function SWEP:Think()
self.Cd = timer.RepsLeft( "APCD" ..self.Owner:UserID() )
self.Activeskil = timer.RepsLeft( "APregen" ..self.Owner:UserID() )
...
function SWEP:DrawHUD()	
	...
	surface.SetMaterial(Material( "skilliconarmoraura.png" ))
