...
SWEP.PrintName = "Heal aura"
SWEP.Author = "KTKycT (Comet)"
SWEP.Slot = 4
SWEP.SlotPos = 0
SWEP.Description = "AOE лечение союзников."
SWEP.Contact = ""
SWEP.Purpose = ""
...
if timer.RepsLeft( "OVHCD" ..self.Owner:UserID() ) != nil or timer.RepsLeft( "ULHCD" ..self.Owner:UserID() ) != nil then
...
timer.Create( "HPregen" ..self.Owner:UserID(), 1, 15, function ()
	...
	for _, hply in pairs(targetheal) do
		if hply:IsPlayer() and hply:Alive() then 
			needheal = hply:GetMaxHealth()
					hply:SetHealth( math.Clamp(hply:Health() + 4, 0, needheal))
			end
		end
 ef = EffectData()
...
timer.Create( "HPCD" ..self.Owner:UserID(), 1, 45, function () end) else return end
end
end
...
function SWEP:Think()
self.Cd = timer.RepsLeft( "HPCD" ..self.Owner:UserID() )
self.Activeskil = timer.RepsLeft( "HPregen" ..self.Owner:UserID() )
...
function SWEP:DrawHUD()	
	...
	surface.SetMaterial(Material( "skilliconhealaura.png" ))
