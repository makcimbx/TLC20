...
SWEP.PrintName = "Overheal aura"
SWEP.Author = "KTKycT (Comet)"
SWEP.Slot = 4
SWEP.SlotPos = 0
SWEP.Description = "AOE оверлечение союзников(необходимо макс у союзников)."
SWEP.Contact = ""
SWEP.Purpose = ""
...
if timer.RepsLeft( "HPCD" ..self.Owner:UserID() ) != nil or timer.RepsLeft( "ULHCD" ..self.Owner:UserID() ) != nil then
...
timer.Create( "OVHeal" ..self.Owner:UserID(), 1, 15, function ()
	targetheal = ents.FindInSphere(self.Owner:GetPos(), 140)
	for _, hply in pairs(targetheal) do
		if hply:IsPlayer() and hply:Alive() and hply:Health() >= hply:GetMaxHealth() then 
			needheal = hply:GetMaxHealth()
					hply:SetHealth( math.Clamp(hply:Health() + 7, 0, needheal + 50))
			end
		end
 ef = EffectData()
...
timer.Create( "OVHCD" ..self.Owner:UserID(), 1, 45, function () end) else return end
end
end
...
function SWEP:Think()
self.Cd = timer.RepsLeft( "OVHCD" ..self.Owner:UserID() )
self.Activeskil = timer.RepsLeft( "OVHeal" ..self.Owner:UserID() )
...
function SWEP:DrawHUD()	
	...
	surface.SetMaterial(Material( "skilliconhealaura2.png" ))
