...
function PANEL:Init()
	...
	self.terminateButton:SetText( "Закрыть" )
...
function PANEL:Paint( w, h )
	...
	if self.reconnectingType == TYPE_WAITFORSERVER then
		...
		if self.isServerOnline then
			...
			draw.SimpleText(
				"Сервер снова в онлайне!", 'acrashscreen_big',
			...
			draw.SimpleText(
				"Вы будете автоматически переподключены через " .. self.reconnectingInTime .. " секунд.", 'acrashscreen_small',
			...
			draw.SimpleText(
				"Сервер не отвечает", 'acrashscreen_big',
				w*0.5 + 2, 86 + 2, Color( 0, 0, 0 ),
				TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
	...
	elseif self.reconnectingType == TYPE_DELAYED then
		...
		draw.SimpleText( -- Outline
			"Автоматическое переподключение через " .. self.reconnectingInTime .. " секунд.", 'acrashscreen_small',
			w*0.5 + 2, 120 + 2, Color( 0, 0, 0 ),
			TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		draw.SimpleText(
			"Автоматическое переподключение через " .. self.reconnectingInTime .. " секунд.", 'acrashscreen_small',
