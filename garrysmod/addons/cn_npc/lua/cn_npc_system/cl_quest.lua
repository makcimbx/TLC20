...
local PANEL = {}
	...
	function PANEL:Init()
		...
		self:SetSize(640, 340)
		self:Center()
		self:SetPos(self.x, ScrH()*0.525)
		self:MakePopup()
	...
	function PANEL:addOption(text, callback, isLeave,f, a,name,hard,m,x)
		...
		if (isLeave) then
			button.DoClick = function() 
				...
				if(f!=nil)then
					if(hard==nil)then
						hook.Call( f, nil, a,name )
					else
						hook.Call( f, nil, a,name,hard,m,x )
					end
				end
			end
		else
	...
	function PANEL:setup(uniqueID)
		...
		if(data.prop==nil)then
			...
			self.model.Entity:SetPos(self.model.Entity:GetPos() - Vector(30, 30, -3))
		end
