...
function NPC:onStart()
	...
	if(time=="0" or time==nil)then
		if(take=="0" or take==nil)then
			self:addText("Здравия желаю солдат! Сделай обход предназначенных для тебя частей венатора.")
			...
			if(make=="0" or make==nil)then
				if(id3==self.uniqueID)then
					...
					self:addOption("Я не могу сейчас выполнить задание!.", function()
						self:addText("Серьёзно? Ладно, возвращайся позже.")
