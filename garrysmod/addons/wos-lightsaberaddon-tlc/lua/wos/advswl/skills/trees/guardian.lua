
--[[-------------------------------------------------------------------
	Lightsaber Force Powers:
		The available powers that the new saber base uses.
			Powered by
						  _ _ _    ___  ____  
				__      _(_) | |_ / _ \/ ___| 
				\ \ /\ / / | | __| | | \___ \ 
				 \ V  V /| | | |_| |_| |___) |
				  \_/\_/ |_|_|\__|\___/|____/ 
											  
 _____         _                 _             _           
|_   _|__  ___| |__  _ __   ___ | | ___   __ _(_) ___  ___ 
  | |/ _ \/ __| '_ \| '_ \ / _ \| |/ _ \ / _` | |/ _ \/ __|
  | |  __/ (__| | | | | | | (_) | | (_) | (_| | |  __/\__ \
  |_|\___|\___|_| |_|_| |_|\___/|_|\___/ \__, |_|\___||___/
                                         |___/             
----------------------------- Copyright 2017, David "King David" Wiltos ]]--[[
							  
	Lua Developer: King David
	Contact: http://steamcommunity.com/groups/wiltostech
		
-- Copyright 2017, David "King David" Wiltos ]]--


local TREE = {}

--Name of the skill tree
TREE.Name = "Защитник"

--Description of the skill tree
TREE.Description = ""

--Icon for the skill tree ( Appears in category menu and above the skills )
TREE.TreeIcon = "wos/skilltrees/forms/aggressive.png"

--What is the background color in the menu for this 
TREE.BackgroundColor = Color( 255, 0, 0, 25 )

--How many tiers of skills are there?
TREE.MaxTiers = 6

--Add user groups that are allowed to use this tree. If anyone is allowed, set this to FALSE ( TREE.UserGroups = false )
TREE.UserGroups = false

TREE.TeamAllowed = {
	["Guardian"] = true,
	["Echo"] = true,
	["kenobi"] = true,
	["windu"] = true,
	["koon"] = true,
	["skywalker"] = true,
	["secura"] = true,
	["mundi"] = true,
	["saeseetiin"] = true,
	["tano"] = true,
	["tano22"] = true,
	["tano3"] = true,
	["tano4"] = true,
	["tano6"] = true,
	["tano7"] = true,
	["jedimg"] = true,
}
TREE.Tier = {}

--Tier format is as follows:
--To create the TIER Table, do the following
--TREE.Tier[ TIER NUMBER ] = {} 
--To populate it with data, the format follows this
--TREE.Tier[ TIER NUMBER ][ SKILL NUMBER ] = DATA
--Name, description, and icon are exactly the same as before
--PointsRequired is for how many skill points are needed to unlock this particular skill
--Requirements prevent you from unlocking this skill unless you have the pre-requisite skills from the last tiers. If you are on tier 1, this should be {}
--OnPlayerSpawn is a function called when the player just spawns
--OnPlayerDeath is a function called when the player has just died
--OnSaberDeploy is a function called when the player has just pulled out their lightsaber ( assuming you have SWEP.UsePlayerSkills = true )


TREE.Tier[1] = {}
TREE.Tier[1][1] = {
	Name = "Навык владения световым мечем IV",
	Description = "Повышает наносимый урон световым мечем на 20.",
	Icon = "wos/skilltrees/ravager/combatant.png",
	PointsRequired = 3,
	Requirements = {},
	--OnPlayerSpawn = function( ply ) end,
	--OnPlayerDeath = function( ply ) end,
	OnSaberDeploy = function( wep ) wep.SaberDamage = wep.SaberDamage + 20 end,
}

TREE.Tier[1][2] = {
	Name = "Улучшение физических способностей I",
	Description = "Увеличивает здоровье на 100",
	Icon = "wos/skilltrees/ravager/strength.png",
	PointsRequired = 2,
	Requirements = {},
	OnPlayerSpawn = function( ply ) ply:SetHealth( ply:Health() + 100) ply:SetMaxHealth(ply:GetMaxHealth() + 100) end,
	--OnPlayerSpawn = function( ply ) end,
	--OnPlayerDeath = function( ply ) end,
	--OnSaberDeploy = function( wep ) end,
}

TREE.Tier[1][3] = {
	Name = "Форма Джар'Кай",
	Description = "Изучите 1 стойку формы Джар'Кай",
	Icon = "wos/skilltrees/forms/arrogant.png",
	PointsRequired = 35,
	Requirements = {},
	--OnPlayerSpawn = function( ply ) end,
	--OnPlayerDeath = function( ply ) end,
	OnSaberDeploy = function( wep ) wep:AddForm( "Jar'Kai", 1 ) end,
}

TREE.Tier[2] = {}
TREE.Tier[2][1] = {
	Name = "Навык владения световым мечем V",
	Description = "Повышает наносимый урон световым мечем на 30.",
	Icon = "wos/skilltrees/ravager/combatant.png",
	PointsRequired = 5,
	Requirements = {
	[1] = { 1 },
	},
	--OnPlayerSpawn = function( ply ) end,
	--OnPlayerDeath = function( ply ) end,
	OnSaberDeploy = function( wep ) wep.SaberDamage = wep.SaberDamage + 30 end,
}

TREE.Tier[2][2] = {
	Name = "Улучшение физических способностей II",
	Description = "Увеличивает здоровье на 150",
	Icon = "wos/skilltrees/ravager/strength.png",
	PointsRequired = 5,
	Requirements = {
	[1] = { 2 },
	},
	OnPlayerSpawn = function( ply ) ply:SetHealth( ply:Health() + 150) ply:SetMaxHealth(ply:GetMaxHealth() + 150) end,
	--OnPlayerSpawn = function( ply ) end,
	--OnPlayerDeath = function( ply ) end,
	--OnSaberDeploy = function( wep ) end,
}

TREE.Tier[2][3] = {
	Name = "Форма Джар'Кай",
	Description = "Изучите 2 стойку формы Джар'Кай",
	Icon = "wos/skilltrees/forms/arrogant.png",
	PointsRequired = 50,
	Requirements = {
	[1] = { 3 },
	},
	--OnPlayerSpawn = function( ply ) end,
	--OnPlayerDeath = function( ply ) end,
	OnSaberDeploy = function( wep ) wep:AddForm( "Jar'Kai", 2 ) end,
}

TREE.Tier[3] = {}
TREE.Tier[3][1] = {
	Name = "Навык владения световым мечем VI",
	Description = "Повышает наносимый урон световым мечем на 50.",
	Icon = "wos/skilltrees/ravager/combatant.png",
	PointsRequired = 7,
	Requirements = {
	[2] = { 1 },
	},
	--OnPlayerSpawn = function( ply ) end,
	--OnPlayerDeath = function( ply ) end,
	OnSaberDeploy = function( wep ) wep.SaberDamage = wep.SaberDamage + 50 end,
}

TREE.Tier[3][2] = {
	Name = "Улучшение физических способностей III",
	Description = "Увеличивает здоровье на 250",
	Icon = "wos/skilltrees/ravager/strength.png",
	PointsRequired = 7,
	Requirements = {
	[2] = { 2 },
	},
	OnPlayerSpawn = function( ply ) ply:SetHealth( ply:Health() + 250) ply:SetMaxHealth(ply:GetMaxHealth() + 250) end,
	--OnPlayerSpawn = function( ply ) end,
	--OnPlayerDeath = function( ply ) end,
	--OnSaberDeploy = function( wep ) end,
}

TREE.Tier[3][3] = {
	Name = "Форма Джар'Кай",
	Description = "Изучите 3 стойку формы Джар'Кай",
	Icon = "wos/skilltrees/forms/arrogant.png",
	PointsRequired = 75,
	Requirements = {
	[2] = { 3 },
	},
	--OnPlayerSpawn = function( ply ) end,
	--OnPlayerDeath = function( ply ) end,
	OnSaberDeploy = function( wep ) wep:AddForm( "Jar'Kai", 3 ) end,
}

TREE.Tier[4] = {}
TREE.Tier[4][1] = {
	Name = "Улучшение защиты I",
	Description = "Снижает затраты силы для отражения атак на 25%",
	Icon = "wos/skilltrees/ravager/scars.png",
	PointsRequired = 5,
	Requirements = {
	[3] = { 1, 2 },
	},
	--OnPlayerSpawn = function( ply ) end,
	--OnPlayerDeath = function( ply ) end,
	OnSaberDeploy = function( wep ) wep.BlockDrainRate = wep.BlockDrainRate*0.75 end,
}


TREE.Tier[5] = {}
TREE.Tier[5][1] = {
	Name = "Улучшение защиты IГ",
	Description = "Снижает затраты силы для отражения атак на 50%",
	Icon = "wos/skilltrees/ravager/scars.png",
	PointsRequired = 10,
	Requirements = {
	[4] = { 1 },
	},
	--OnPlayerSpawn = function( ply ) end,
	--OnPlayerDeath = function( ply ) end,
	OnSaberDeploy = function( wep ) wep.BlockDrainRate = wep.BlockDrainRate*0.50 end,
}

TREE.Tier[6] = {}
TREE.Tier[6][1] = {
	Name = "Улучшение защиты IIГ",
	Description = "Снижает затраты силы для отражения атак на 75%",
	Icon = "wos/skilltrees/ravager/scars.png",
	PointsRequired = 20,
	Requirements = {
	[5] = { 1 },
	},
	--OnPlayerSpawn = function( ply ) end,
	--OnPlayerDeath = function( ply ) end,
	OnSaberDeploy = function( wep ) wep.BlockDrainRate = wep.BlockDrainRate*0.25 end,
}

wOS:RegisterSkillTree( TREE )